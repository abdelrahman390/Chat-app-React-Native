...
export default function LoginPage() {
  ...
  const handleRegister = async () => {
    ...
    if (registerUsername.length !== 0 && registerPassword.length !== 0 && registerPassword === registerPasswordConfirm) {
        ...
        {login == "register" ? (
          <View style={styles.box}>
            ...
            <View style={styles.inputsParent}>
              ...
              />
              <TouchableOpacity style={styles.iconContainer} onPress={() => setLoginPassShow((prevValue) => !prevValue)}>
                <Image source={require('../../assets/imgs/hide.png')} style={styles.hideIcon} />
              </TouchableOpacity>
            </View>
            <Text style={[styles.message, { display: loginAlarm ? 'flex' : 'none' }]}>{loginAlarm ? "User name or Password are incorrect" : null}</Text>
            <Text style={[styles.message, { display: loginServerAlarm ? 'flex' : 'none' }]}>Server Error</Text>
            <TouchableOpacity style={styles.button} onPress={handleLogin}>
              <Text style={styles.buttonText}>{loginLoading ? "Loading..." : 'Login'}</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <View style={styles.box}>
            ...
            />
            <Text style={[styles.message, { display: registerAlarm ? 'flex' : 'none' }]}>User Name Already Taken</Text>
            <View style={styles.inputsParent}>
              <TextInput
                style={styles.inputs}
                placeholder="Password"
              ...
              />
              <TouchableOpacity style={styles.iconContainer} onPress={() => setRegisterPassShow((prevValue) => !prevValue)}>
                <Image source={require('../../assets/imgs/hide.png')} style={styles.hideIcon} />
              </TouchableOpacity>
            </View>
            <View style={styles.inputsParent}>
              ...
              />
              <TouchableOpacity style={styles.iconContainer} onPress={() => setRegisterPassConfirmShow((prevValue) => !prevValue)}>
                <Image source={require('../../assets/imgs/hide.png')} style={styles.hideIcon} />
              </TouchableOpacity>
            </View>
            <Text style={[styles.passMessage, { display: registerPassAlarm ? 'flex' : 'none' }]}>The two passwords are not match</Text>
            <TouchableOpacity style={styles.button} onPress={handleRegister}>
              <Text style={styles.buttonText}>{registerLoading ? "Loading..." : 'Sign Up'}</Text>
            </TouchableOpacity>
          </View>
        )}
        <TouchableOpacity style={styles.buttonChangeLogin} onPress={() => setLogin((prevState) => (prevState === 'register' ? 'login' : 'register'))} >
